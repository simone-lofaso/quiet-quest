...
const SearchPage = () => {
  ...
  {
    /*USES GEOAPIFY PLACES API TO FIND SEARCH CATEGORIES*/
  }
  const searchPlaces = async () => {
    ...
    try {
      ...
      if (data.features && data.features.length > 0) {
        ...
        console.log(EXPO_PUBLIC_GEOAPIFY_API_KEY);
      ...
      <View style={styles.resultsContainer}>
        {loading ? (
          ...
          <FlatList
            data={places}
            keyExtractor={(item) =>
              item.properties.place_id || item.properties.name
            }
            renderItem={({ item }) => (
              <View style={styles.placeItem}>
                ...
                <Text style={styles.placeCategory}>
                  Categories: {item.properties.categories?.join(", ")}
                </Text>
